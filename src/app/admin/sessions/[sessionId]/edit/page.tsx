...
export default function EditSessionPage() {
  ...
  const loadSession = useCallback(async () => {
    try {
      ...
      if (!json.ok) throw new Error(json.error?.message || "세션을 불러올 수 없습니다");
      const s = json.data;
      setTitle(s.title);
      ...
      setBlocks(
        (s.blocks || [])
          .filter((b: BlockInput & { id?: string }) => b.type === "TIMELINE" || b.type === "FLOW")
          .map((b: BlockInput & { id?: string; type: string }) => ({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json();
      if (!data.ok) throw new Error(data.error?.message || "저장에 실패했습니다");
  ...
  async function handleDelete() {
    if (!confirm("정말로 이 세션을 삭제하시겠습니까? 모든 블록, 자료, 제출물이 함께 삭제됩니다.")) return;

    try {
      const res = await fetch(`/api/sessions/${sessionId}`, { method: "DELETE" });
      const data = await res.json();
      if (!data.ok) throw new Error(data.error?.message || "삭제에 실패했습니다");
      router.push("/admin/sessions");
    } catch (err) {
      setError((err as Error).message);
    }
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-800">세션 편집</h1>
        <button
